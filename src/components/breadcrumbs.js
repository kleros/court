...
const Breadcrumbs = ({ activeIndex, breadcrumbs, className, colorIndex, large, onClick }) => {
  const handleClick = useCallback((e) => onClick(Number(e.currentTarget.id)), [onClick]);
  ...
  return (
    <StyledDiv className={className} large={large}>
      {(Array.isArray(breadcrumbs) ? breadcrumbs : [breadcrumbs]).map((b, i) => (
        ...
        >
          <StyledBreadcrumb
            className={`${["first", "second", "third"][(colorIndex === null ? i : colorIndex) % 3]}-fill`}
            large={String(large)}
          />
...
Breadcrumbs.propTypes = {
  ...
  breadcrumbs: PropTypes.oneOfType([PropTypes.arrayOf(PropTypes.node.isRequired).isRequired, PropTypes.node.isRequired])
    .isRequired,

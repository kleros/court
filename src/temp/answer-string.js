...
export const getAnswerString = (rulingOptions, votesData, hex = false) => {
  const questionJson = {
  ...
  if (isNumericAnswer) {
    if (hex) {
      return realitioLibQuestionFormatter.padToBytes32(toBN(votesData.voted).sub(toBN("1")).toString(16));
    }
